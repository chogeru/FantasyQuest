    ...
    public class PrefabsTreeView : BaseOccludeeCategoryTreeView
    {
        ...
        private DragAndDropVisualMode handleDragAndDropPrefabs(DragAndDropArgs args, Object[] objectReferences)
        {
            ...
            for (int i = 0; i < objectReferences.Length; i++)
            {
                ...
                if (prefab.GetComponent<Occludee>() != null)
                {
                    Debug.LogError($"Unable to register prefab '{prefab.name}' - prefab already has an Occldudee component attached and may be part of an existing configuration.");
                    return DragAndDropVisualMode.Rejected;
                }
